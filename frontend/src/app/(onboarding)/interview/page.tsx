...
export default function InterviewPage() {
  ...
  async function startInterview() {
    ...
    setLoading(true)
    try {
      const res = await api.onboarding.sendInterviewMessage(
        "I'm ready to begin."
      )
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (res.is_complete) {
        await refreshUser()
        setTimeout(() => router.push('/onboarding/goal'), 1200)
      }
    } catch (err) {
      setMessages([
        ...newMessages,
      ...
      <div className="text-center max-w-lg mx-auto">
        ...
        >
          ...
          <button
            ...
            className="btn btn-primary px-10 h-13 text-base"
          >
            Begin the interview
          </button>
        </motion.div>
        ...
        <AnimatePresence initial={false}>
          {messages.map((msg, i) => (
            ...
            >
              ...
              <div
                className={`max-w-[85%] px-4 py-3 rounded-2xl text-[0.9375rem] leading-relaxed ${
                  msg.role === 'user'
